...
public:
    ...
    void draw_vor(std::vector<glm::vec4> positions, glm::mat4 projection, glm::mat4 view);
private:
    PointSprite sprite;
};
...
void DrawParticle::draw_vor(std::vector<glm::vec4> positions, glm::mat4 projection, glm::mat4 view) {
    unsigned int sprVBO;
	glGenBuffers(1, &sprVBO);
	int VBO_size = positions.size() * 4 * sizeof(float);        // vec4
	glBindBuffer(GL_ARRAY_BUFFER, sprVBO);
	...
	sprite.draw_vor(projection, view);
